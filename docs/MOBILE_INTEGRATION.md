...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Authentication | Implemented: email/password login, signup, logout | <code>app/page.tsx</code>, <code>app/login/page.tsx</code>, <code>lib/supabase.ts</code> | Supabase Auth | Один user identity може володіти user-scoped data | Forms, modal, Next.js redirects | Native screens, deep links, secure token storage | Remote Auth settings: <code>REMOTE STATE: UNKNOWN</code> |
| Session behavior | Initial <code>getSession()</code> check і route policy implemented | <code>lib/auth-route-policy.ts</code>, protected pages | Supabase session | Protected content не показується до завершення auth check | <code>router.replace</code>, browser client | Refresh/expiry, app resume, secure persistence | <code>onAuthStateChange</code> не знайдено; recovery not implemented |
...
Web Supabase client використовує <code>NEXT_PUBLIC_SUPABASE_URL</code> і <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code>. Реалізовано <code>signInWithPassword()</code>, <code>signUp()</code> і <code>signOut()</code>. Nickname під час основної registration передається також у user metadata і записується до <code>profiles</code>, якщо session доступна.

Mobile може використовувати ті самі Auth identities лише після підтвердження production project, redirect policy та email delivery. Password recovery, email confirmation behavior, OAuth і branded email не можна виводити з поточного Web code як готові flows.
