...
export default function GarageDashboard() {
  ...
  const handleUpdateRoutine = async (e: React.FormEvent) => {
    ...
    if (!histError) {
      ...
      fetchData(userId);
    } else alert(t.garage.errorPrefix + histError.message);
  ...
  const handleDeleteRule = async (id: number) => {
    if (!userId) return;
    if (confirm(t.garage.deleteConfirmation)) {
  ...
  if (!userId) {
    return <div className="min-h-screen bg-[#121212] text-white flex items-center justify-center">{t.garage.checkingAccess}</div>;
  }
    ...
    <div className="min-h-screen bg-[#121212] text-white p-4 md:p-8 relative">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-[#1e1e24] to-[#1a1a20] p-4 rounded-xl border border-gray-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6 shadow-md">
          <div>
            <h2 className="text-lg font-bold text-blue-400 mb-0.5">{t.garage.odometerTitle}</h2>
            <p className="text-xs text-gray-500">{t.garage.odometerHint}</p>
          </div>
          <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            />
            <span className="text-gray-400 font-bold text-sm">{t.common.km}</span>
          </div>
        </div>
          ...
          <form onSubmit={handleAddRule} className="p-4 bg-gradient-to-br from-blue-900/10 to-[#1e1e24] border border-blue-900/30 rounded-xl shadow-md">
            <div className="mb-3">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-gray-300 text-sm font-medium">{t.garage.hasReplacementInterval.replace("{unit}", t.common.km)}</span>
              </label>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              <div className={hasInterval ? "" : "sm:col-span-2"}>
                <label className="block text-xs text-gray-400 mb-1">{t.garage.name}</label>
              ...
              {hasInterval && (
                <div>
                  <label className="block text-xs text-gray-400 mb-1">{t.garage.resourceLabel.replace("{unit}", t.common.km)}</label>
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">{t.garage.lastReplacementMileage}</label>
            ...
            <button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 hover:bg-blue-500 text-white py-2.5 rounded-lg text-sm font-bold transition">{t.garage.saveToGarage}</button>
          </form>
        </div>
          ...
          <form onSubmit={handleAddRepair} className="p-4 bg-gradient-to-br from-red-900/10 to-[#1e1e24] border border-red-900/30 rounded-xl shadow-md">
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-4">
              <div className="sm:col-span-2">
                <label className="block text-xs text-gray-400 mb-1">{t.garage.repairName}</label>
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">{t.garage.date}</label>
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">{t.garage.mileageLabel.replace("{unit}", t.common.km)}</label>
            ...
            <div className="mb-4 w-full sm:w-1/2">
              <label className="block text-xs text-gray-400 mb-1">{t.garage.costLabel.replace("{currency}", t.common.currency)}</label>
            ...
            <button type="submit" disabled={isSubmitting} className="w-full bg-red-600 hover:bg-red-500 text-white py-2.5 rounded-lg text-sm font-bold transition">{t.garage.addToExpenses}</button>
          </form>
        </div>

        <div className="mb-10">
          <h2 className="text-lg font-bold mb-4 flex items-center gap-2">{t.garage.partsStatus}</h2>
          {isLoading ? (
            <p className="text-gray-500 text-sm">{t.common.loading}</p>
          ) : rules.length === 0 ? (
            <div className="bg-[#1e1e24] p-6 rounded-xl border border-gray-800 text-center text-sm text-gray-500">
              {t.garage.emptyGarage}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {rules.map((rule) => {
                ...
                let statusText = t.garage.statusOk;

                if (isTrackOnly) {
                  barColor = "bg-blue-500/50";
                  textColor = "text-blue-400";
                  statusText = t.garage.statusTracking;
                } else {
                  if (kmsDrivenSinceChange >= rule.interval_km) {
                    barColor = "bg-red-500 animate-pulse";
                    textColor = "text-red-500";
                    statusText = t.garage.statusReplace;
                  } else if (percentUsed >= 80) {
                    barColor = "bg-yellow-500";
                    textColor = "text-yellow-500";
                    statusText = t.garage.statusPrepare;
                  }
                }
                ...
                return (
                  <div key={rule.id} className={`bg-[#1e1e24] border ${isTrackOnly ? 'border-blue-900/30' : 'border-gray-800'} rounded-xl p-4 shadow-md relative flex flex-col justify-between`}>
                    <div>
                      ...
                      <div className="flex justify-between items-end mb-2">
                        <div className="text-xs text-gray-400 leading-tight">
                          {isTrackOnly
                            ? t.garage.unlimited
                            : t.garage.resourceValue
                                .replace("{value}", String(rule.interval_km))
                                .replace("{unit}", t.common.km)}
                        </div>
                        <div className="text-right leading-tight">
                          ...
                          <span className="text-[10px] text-gray-500 uppercase tracking-wide">
                            {isTrackOnly ? t.garage.covered : t.garage.remaining}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between text-[10px] text-gray-500 mb-3">
                        <span>{isTrackOnly ? t.garage.untilWear : t.garage.drivenValue.replace("{value}", kmsDrivenSinceChange.toFixed(0))}</span>
                        <span>{t.garage.lastReplacementValue.replace("{value}", rule.last_change_km.toFixed(0))}</span>
                      </div>
                      ...
                      {intervals && intervals.length > 0 && !isTrackOnly && (
                         <div className="bg-[#2a2a35] rounded p-2 text-[10px] text-gray-400 mb-3 border border-gray-700/50">
                           <span className="block text-gray-500 mb-0.5">{t.garage.actualIntervals}</span>
                    ...
                    >
                      <span>⚙️</span> {t.garage.recordReplacement}
                    </button>
                  </div>
                );
              })}
        ...
        <div>
          <h2 className="text-lg font-bold mb-4 flex items-center gap-2">{t.garage.statsHistory}</h2>
          
          <div className="grid grid-cols-2 gap-3 mb-6">
            <div className="bg-gradient-to-br from-gray-800 to-[#1e1e24] p-4 rounded-xl border border-gray-700">
              <h3 className="text-gray-400 mb-0.5 text-[10px] uppercase tracking-wide">{t.garage.totalSpent}</h3>
              <p className="text-2xl font-black text-white">{totalCost.toFixed(2)} <span className="text-sm text-gray-500">{t.common.currency}</span></p>
            </div>
            <div className="bg-gradient-to-br from-purple-900/20 to-[#1e1e24] p-4 rounded-xl border border-purple-900/30">
              <h3 className="text-gray-400 mb-0.5 text-[10px] uppercase tracking-wide">{t.garage.monthlyAverage}</h3>
              <p className="text-2xl font-black text-purple-400">{monthlyAvgCost.toFixed(2)} <span className="text-sm">{t.garage.perMonth.replace("{currency}", t.common.currency)}</span></p>
            </div>
          </div>
          ...
          <div className="bg-[#1e1e24] rounded-xl border border-gray-800 overflow-hidden">
            <div className="p-3 border-b border-gray-800 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-[#2a2a35]/50 gap-3">
              <h2 className="text-sm font-medium">{t.garage.serviceHistory}</h2>
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                >
                  <option value="all">{t.garage.allRecords}</option>
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr className="bg-[#2a2a35] text-gray-400 text-xs">
                    <th className="p-3 font-medium">{t.garage.date}</th>
                    <th className="p-3 font-medium">{t.garage.type}</th>
                    <th className="p-3 font-medium">{t.garage.workPerformed}</th>
                    <th className="p-3 font-medium">{t.garage.mileage}</th>
                    <th className="p-3 font-medium">{t.garage.distanceCoveredLabel.replace("{unit}", t.common.km)}</th>
                    <th className="p-3 font-medium">{t.garage.cost}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800 text-sm">
                  {filteredHistory.length === 0 ? (
                    <tr><td colSpan={6} className="p-6 text-center text-gray-500 text-xs">{t.garage.emptyHistory}</td></tr>
                  ) : (
                    filteredHistory.map((record) => (
                      <tr key={record.id} className="hover:bg-[#2a2a35] transition">
                        <td className="p-3 text-gray-400 text-xs">{new Date(record.date).toLocaleDateString(dateLocale)}</td>
                        <td className="p-3">
                          {record.service_type === "routine" 
                            ? <span className="text-[10px] bg-blue-900/50 text-blue-300 px-1.5 py-0.5 rounded">{t.garage.routineType}</span>
                            : <span className="text-[10px] bg-red-900/50 text-red-300 px-1.5 py-0.5 rounded">{t.garage.repairType}</span>
                          }
                        </td>
                        ...
                        <td className="p-3 font-bold text-white text-xs">{record.cost.toFixed(2)} {t.common.currency}</td>
                      </tr>
                    ))
                  )}
                </tbody>
                  ...
                  <tfoot className="bg-[#2a2a35]/80 text-white font-bold text-xs border-t border-gray-700">
                    <tr>
                      <td colSpan={5} className="p-3 text-right uppercase tracking-wider text-gray-400">
                        {t.garage.selectedTotal}
                      </td>
                      <td className="p-3 text-blue-400 text-sm">
                        {filteredTotalCost.toFixed(2)} {t.common.currency}
                      </td>
                    </tr>
                  </tfoot>
                )}
      ...
      {updateModalRule && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1e1e24] rounded-2xl border border-blue-900/50 w-full max-w-sm p-5 shadow-2xl animate-fadeIn">
            <h2 className="text-xl font-bold text-white mb-1">{t.garage.replacementTitle.replace("{name}", updateModalRule.name)}</h2>
            <p className="text-gray-400 text-xs mb-5">{t.garage.replacementDescription}</p>
            
            <form onSubmit={handleUpdateRoutine}>
              <div className="space-y-3 mb-5">
                <div>
                  <label className="block text-xs text-gray-400 mb-1">{t.garage.replacementDate}</label>
              ...
              <div className="flex gap-2">
                <button type="submit" disabled={isSubmitting} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-2.5 rounded-lg text-sm font-bold transition">{t.common.save}</button>
                <button type="button" onClick={() => setUpdateModalRule(null)} className="px-4 py-2.5 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm font-bold transition">{t.common.cancel}</button>
              </div>
            </form>
          </div>
        </div>
