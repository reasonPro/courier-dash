...
export function WorkComparisonBadge({ comparison, metric, lang }: { comparison: WorkComparison; metric: "income" | "rate" | "daily"; lang: WorkLanguage }) {
  const id = useId();
  ...
  const unit = metric === "daily" ? metricTranslations[lang].day : metric === "rate" ? copy.unit : "PLN";
  const title = metric === "daily" ? metricTranslations[lang].daily : copy[metric];
  const p = comparison.periods;
  return <>
    <button type="button" popoverTarget={id} aria-label={`${copy.title}: ${title} ${badge}`} aria-haspopup="dialog" aria-expanded={open} aria-controls={id}
    ...
    <div id={id} ref={ref} popover="auto" role="dialog" aria-labelledby={`${id}-title`} tabIndex={-1}
      ...
      <h4 id={`${id}-title`} className="mb-3 font-bold text-white">{copy.title}: {title}</h4>
